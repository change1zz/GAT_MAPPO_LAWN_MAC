...
class CSMAAgent:
    ...
    def __init__(self, num_slots: int, num_nodes: int, *, num_channels: int = 1, cw_min: int = 4, cw_max: int = 64) -> None:
    ...
    def select_actions(self, obs_x: np.ndarray, last_status: np.ndarray, rng: np.random.Generator, *, max_tx: int = 1) -> np.ndarray:
        ...
        if n_attempt > 0:
            idx = np.where(attempt)[0].astype(int)
            for i in idx:
                if l == 1:
                    actions[i, 0] = int(rng.integers(low=0, high=(self.A - 1), dtype=np.int64))
                else:
